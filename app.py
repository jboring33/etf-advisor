...
- Full 10-rule suite with adjustable parameters and scoring weights.
- Fixed Alpha vs SPY benchmark logic in batch screening.
- Single Symbol view with dynamic scoring explanations and investment theses.
"""
...
# ==============================================================================
# SCORING & RULE EVALUATION ENGINE (10 RULES)
# ==============================================================================
...
def evaluate_rules(df: pd.DataFrame, benchmark_df: pd.DataFrame, params: dict):
    ...
    # Rule 4: Relative Strength vs Benchmark (SPY) - Explicit Alpha Fix
    alpha_pct = 0.0
    ...
    # Rule 8: RSI Band Filter (Not Overbought / Not Oversold)
...
# ==============================================================================
# SIDEBAR: RULE PARAMETERS & SCORE WEIGHT CONFIGURATION (10 RULES)
# ==============================================================================

st.sidebar.header("⚙️ Rule & Weight Configurator")
st.sidebar.caption("Set parameters and adjust the point values assigned to each of the 10 rules.")
...
weight_ma_val = st.sidebar.slider("Trend Weight (pts)", 0, 50, 15)
...
weight_perf_val = st.sidebar.slider("Performance Weight (pts)", 0, 50, 10)

st.sidebar.subheader("3. Money Flow Rule")
min_flow_val = st.sidebar.slider("Min Money Flow Score", 0, 100, 50)
weight_flow_val = st.sidebar.slider("Money Flow Weight (pts)", 0, 50, 10)
...
weight_dd_val = st.sidebar.slider("Drawdown Weight (pts)", 0, 50, 10)
...
weight_52w_val = st.sidebar.slider("52W High Weight (pts)", 0, 50, 10)
...
weight_rsi_val = st.sidebar.slider("RSI Weight (pts)", 0, 50, 10)
...
weight_sharpe_val = st.sidebar.slider("Sharpe Ratio Weight (pts)", 0, 50, 10)
...
weight_atr_val = st.sidebar.slider("ATR Squeeze Weight (pts)", 0, 50, 10)
...
RULE_PARAMS = {
    ...
    "min_rsi": min_rsi_val,
    "max_rsi": max_rsi_val,
    "weight_rsi": weight_rsi_val,
    "min_sharpe": min_sharpe_val,
    "weight_sharpe": weight_sharpe_val,
    "max_atr_pct": max_atr_val,
    "weight_atr": weight_atr_val
}

MAX_POSSIBLE_SCORE = sum([
    weight_ma_val, weight_perf_val, weight_flow_val, weight_rs_val, 
    weight_vol_exp_val, weight_dd_val, weight_52w_val, weight_rsi_val,
    weight_sharpe_val, weight_atr_val
])
...
with tab_screen:
    ...
    st.caption("Enter your list of ETFs to evaluate them against all 10 sidebar rules.")
...
with tab_single:
    ...
    st.caption("Inspect exactly how an individual symbol scores against all 10 active rules.")
